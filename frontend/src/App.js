...
function App() {
  ...
  const reset=()=>{abortRef.current?.abort(); setResult(null); setShowUpgrade(false);};

  const processBill=async(file)=>{
    setLoading(true);
    abortRef.current=new AbortController();
    const form=new FormData(); form.append("bill",file);
    try{
      const res=await fetch(WORKER_URL,{method:"POST",body:form,signal:abortRef.current.signal});
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-blue-50 dark:from-gray-900 dark:to-indigo-950">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        {!result ? <BillUploader onResult={processBill} onLoading={setLoading}/> :
          <>
            <div className="text-center mb-6">
              <button onClick={reset} className="text-indigo-600 dark:text-indigo-400 hover:underline text-sm">← Analyze Another Bill</button>
            </div>
            <ExplanationCard result={result} onUpgrade={()=>setShowUpgrade(true)}/>
            {result.isPaid && result.pages[0]?.structured && <PaidFeatures features={result.pages[0].structured}/>}
          </>
        }
      </main>
      ...
      <section className="max-w-4xl mx-auto px-4 py-12">
        <h2 className="text-3xl font-bold text-center mb-8 bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Your Privacy Is Guaranteed</h2>
